...
app.post('/login', function(req, res){
    if(!user.twofactor || !user.twofactor.secret){ //two factor is not enabled by the user
        ...
        if(!req.headers['x-otp']){
            return res.status(206).send('Please enter the secret key to continue');
        }
        //validate otp
...
//Logged in app
app.get('/jsoneditor.html', function(req, res){
    res.sendFile(path.join(__dirname+'/jsoneditor.html'));
});

app.get('/vue.app.html', function(req, res){
    res.sendFile(path.join(__dirname+'/vue.app.html'));
});
app.listen('3000', ()=>{
